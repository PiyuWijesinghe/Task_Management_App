    ...
    <div class="flex h-screen bg-transparent">
            ...
            <nav class="mt-6 flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent hover:scrollbar-thumb-gray-400 dark:hover:scrollbar-thumb-gray-500">
                <div class="px-4 pb-6">
                    <div class="space-y-1">
                        ...
                        <!-- Postponed Tasks -->
                        <a href="{{ route('tasks.postponed') }}" class="text-gray-700 dark:text-gray-200 hover:bg-white/50 dark:hover:bg-gray-700/50 hover:scale-105 group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 backdrop-blur-sm">
                            <div class="w-8 h-8 bg-gray-100 dark:bg-gray-700 group-hover:bg-gradient-to-r group-hover:from-orange-500 group-hover:to-red-500 rounded-lg flex items-center justify-center mr-3 transition-all duration-200">
                                <svg class="text-gray-500 group-hover:text-white h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- Reports -->
                        <a href="{{ route('reports.index') }}" class="text-gray-700 dark:text-gray-200 hover:bg-white/50 dark:hover:bg-gray-700/50 hover:scale-105 group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 backdrop-blur-sm">
        ...
        <div class="flex-1 overflow-auto bg-transparent">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
                @php
                    $user = Auth::user();
                    // Get both created and assigned tasks with proper query structure
                    $allUserTasks = App\Models\Task::where(function($query) use ($user) {
                        $query->where('user_id', $user->id)
                              ->orWhere('assigned_user_id', $user->id);
                    })->get();
                    
                    $totalTasks = $allUserTasks->count();
                    $pendingTasks = $allUserTasks->where('status', 'Pending')->count();
                    $inProgressTasks = $allUserTasks->where('status', 'In Progress')->count();
                ...
                <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl overflow-hidden shadow-2xl rounded-2xl border border-white/20 dark:border-gray-700/20">
                    <div class="p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <!-- Export PDF Button (go to reports page for filtering) -->
                            <a href="{{ route('reports.index', ['format' => 'pdf']) }}" class="flex items-center p-3 text-white rounded-lg transition duration-200 hover:scale-105 transform" style="background: linear-gradient(135deg, #ef4444 0%, #f59e42 100%);">
                                <svg class="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                </svg>
                                <div>
                                    <h4 class="font-semibold">Export PDF</h4>
                                    <p class="text-xs text-red-100">Filter & export as PDF</p>
                                </div>
                            </a>

                            <!-- Export Excel Button (go to reports page for filtering) -->
                            <a href="{{ route('reports.index', ['format' => 'csv']) }}" class="flex items-center p-3 text-white rounded-lg transition duration-200 hover:scale-105 transform" style="background: linear-gradient(135deg, #22c55e 0%, #38bdf8 100%);">
                                <svg class="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-green-100">Filter & export as Excel/CSV</p>
                                </div>
                            </a>
                        </div>
                    </div>
                                ...
                                <div class="bg-white/50 dark:bg-gray-700/50 backdrop-blur-sm rounded-xl border border-white/20 dark:border-gray-600/20 p-4 hover:bg-white/70 dark:hover:bg-gray-600/70 transition-all duration-300 hover:scale-[1.01] hover:shadow-lg">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex items-center space-x-2 ml-4">
                                            ...
                                            <!-- Comment -->
                                            <a href="{{ route('tasks.show', $task) }}" class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-purple-500 to-pink-600 text-white text-xs font-medium rounded-lg hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <!-- Postpone Button (only for task creator) -->
                                            @if($task->user_id === Auth::id() && $task->status !== 'Completed')
                                            <button onclick="togglePostponeForm('{{ $task->id }}')" 
                                                    class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white text-xs font-medium rounded-lg transition-all duration-200 shadow-md hover:shadow-lg hover:scale-105">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                </svg>
                                                📅 Postpone
                                    ...
                                    <!-- Postpone Form (Hidden by default, only for task creator) -->
                                    @if($task->user_id === Auth::id() && $task->status !== 'Completed')
    ...
    <script>
        function togglePostponeForm(taskId) {
            ...
            if (form.classList.contains('hidden')) {
                // Hide all other postpone forms first
                document.querySelectorAll('[id^="postponeForm-"]').forEach(function(otherForm) {
